...
function updateReportsMapMarkers(reports = []) {
  ...
  if (bounds.length) {
    if (bounds.length === 1) {
      ...
      mapInstance.fitBounds(bounds, { padding: [24, 24], maxZoom: 17 });
    }
  } else {
...
reportForm.addEventListener('reset', () => {
  ...
  formMsg.textContent = '';
  resetUserMarker();
  if (mapInstance && reportsLayer && reportsLayer.getLayers().length === 0) {
    mapInstance.setView(defaultMapCenter, defaultMapZoom);
  }
});
...
async function loadReports() {
  ...
  try {
    ...
    if (!data.reports || data.reports.length === 0) {
      reportsList.innerHTML = '<div class="muted">신고 내역이 없습니다.</div>';
    ...
    reportsList.innerHTML = `<div class="muted">불러오지 못했습니다.<br><small>${escapeHtml(
